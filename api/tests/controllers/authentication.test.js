...
describe("/tokens", () => {
  ...
  test("returns a token when credentials are valid", async () => {
    ...
    const response = await testApp
      .post("/tokens")
      .send({ email: "auth-test@test.com", password: "12345678", fullname: "Someone" });
  ...
  test("doesn't return a token when the user doesn't exist", async () => {
    ...
    const response = await testApp
      .post("/tokens")
      .send({ email: "non-existent@test.com", password: "12345678", fullname: "Someone" });
  ...
  test("doesn't return a token when the wrong password is given", async () => {
    ...
    const response = await testApp
      .post("/tokens")
      .send({ email: "auth-test@test.com", password: "12345679", fullname: "Someone" });
